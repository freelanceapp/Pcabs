...
{
	$this->db->select('a.pickup_area,a.pickup_time,a.pickup_address,a.drop_area,a.item_status,a.status,a.id,a.pickup_date,a.uneaque_id,a.taxi_type,a.amount');
 ...
 $this->db->where('pickup_date', $date);
	...
	$this->db->where_in('a.item_status', array('Completed','')); 
	$this->db->order_by('id', 'desc'); 
    $this->db->limit($per_page,$start);       
	$query = $this->db->get(); 
	$query_pag_data= $query->result();
}else{
	$this->db->select('a.pickup_area,a.pickup_time,a.pickup_address,a.drop_area,a.item_status,a.status,a.id,a.pickup_date,a.uneaque_id,a.taxi_type,a.amount');
	$this->db->from('bookingdetails a'); 
	$this->db->join('driver_details b', 'b.id=a.assigned_for', 'left');
...
$this->db->select('a.pickup_area,a.pickup_address,a.drop_area,a.item_status,a.status,a.id,a.pickup_date,a.uneaque_id,a.taxi_type,a.amount');
	$this->db->from('bookingdetails a'); 
	$this->db->join('driver_details b', 'b.id=a.assigned_for', 'left');
	if($_POST['date'] != 0){
	$date= $_POST['date'];
 $this->db->where('pickup_date', $date);
	...
	$query = $this->db->get(); 
	$result_pag_num= $query->result_array();
...
echo $msg;
	...
	echo "<div><p style='font-family:sans-serif;font-size:15px;margin-top:12px;text-align:center;'>There is no rides </p><div style='text-align:center;'>"; ?>
